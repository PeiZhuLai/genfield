...
for era in "${eraList[@]}"; do
  for mA in "${massList[@]}"; do
    ...
    LOG_FILE="/afs/cern.ch/work/p/pelai/HZa/gridpacks/genfield/massProduce/run3/crab_${era}/HZaTo2l2g_digi_M${mA}/crab_HZa_digi_${era}_M${mA}/crab.log"
    ...
    mkdir -p ./DAS_Names/digi
    # 直接把 DATASET_PATH 寫進檔案（只一行）
    echo "${DATASET_PATH}" > "./DAS_Names/digi/DAS_Names_${era}_M${mA}.txt"
    # echo "Dataset path saved to /DAS_Names/digi/DAS_Names_${era}_M${mA}.txt"
