...
CRAB_TMPL="${CRAB_TMPL_DIR}/crabConfig_AOD_template.py"
...
for era in "${eraList[@]}"; do
  ...
  for mass in "${massList[@]}"; do
    for fraction in "${fractions[@]}"; do
      ...
      # 各 era 在 3_prepareConfig_*.sh 產生的 AOD cfg 命名規則：
      #   ${CFG_BASE_DIR}/HZaTo2l2g_M${MASS}/fraction${FRACTION}/3_AOD_fragment_${ERA}_${FRACTION}.py
      ...
      echo "Submitting CRAB AOD task for ERA=${ERA} (${ERA_ALIAS}), M=${MASS}, frac=${FRACTION}"
