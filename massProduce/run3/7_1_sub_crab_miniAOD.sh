...
for era in "${eraList[@]}"; do
  ...
  for mass in "${massList[@]}"; do
    ...
    export DASFILEBASE="/afs/cern.ch/work/p/pelai/HZa/gridpacks/genfield/massProduce/run3/DAS_Names/AOD"

    cfgName="crab_HZa_${era}_M${mass}_miniAOD.py"
    cfgPath="${miniAOD_CFG_DIR}/${cfgName}"

    if [ "${CRAB_RESUBMIT}" = "1" ]; then
      echo "Resubmitting CRAB miniAOD task for ERA=${ERA}, M=${MASS}"
