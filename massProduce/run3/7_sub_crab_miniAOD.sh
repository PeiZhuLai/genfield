...
CRAB_TMPL="${CRAB_TMPL_DIR}/crabConfig_miniAOD_template.py"
...
for era in "${eraList[@]}"; do
  ...
  for mass in "${massList[@]}"; do
    for fraction in "${fractions[@]}"; do
      ...
      # MINIAOD step（在 13_0_X 下）
      export STEP="MINIAOD"

      # 各 era 在 3_prepareConfig_*.sh 產生的 miniAOD cfg 命名規則：
      #   ${CFG_BASE_DIR}/HZaTo2l2g_M${MASS}/fraction${FRACTION}/4_miniAOD_fragment_${ERA}_${FRACTION}.py
      export CFG_PATH="${CFG_BASE_DIR}/HZaTo2l2g_M${MASS}/fraction${FRACTION}/4_miniAOD_fragment_${ERA}_${FRACTION}.py"

      export OUT_DIR="${OUT_BASE_DIR}/HZaTo2l2g/MINIAOD/M${MASS}/${ERA}"

      cfgName="crab_HZa_${era}_M${mass}_frac${fraction}_MINIAOD.py"
      cfgPath="${CRAB_TMPL_DIR}/${cfgName}"

      cp "${CRAB_TMPL}" "${cfgPath}"

      echo "Submitting CRAB MINIAOD task for ERA=${ERA} (${ERA_ALIAS}), M=${MASS}, frac=${FRACTION}"
