...
for era in "${eraList[@]}"; do
  ...
  for mass in "${massList[@]}"; do
    ...
    # AOD Input: digi
    export DASFILEBASE="/afs/cern.ch/work/p/pelai/HZa/gridpacks/genfield/massProduce/run3/DAS_Names/digi"

    cfgName="crab_HZa_${era}_M${mass}_AOD.py"
    cfgPath="${AOD_CFG_DIR}/${cfgName}"

    if [ "${CRAB_RESUBMIT}" = "1" ]; then
      echo "Resubmitting CRAB AOD task for ERA=${ERA}, M=${MASS}"
      ...
      echo "Submitting CRAB AOD task for ERA=${ERA}, M=${MASS}"
      rm -f "${cfgPath}"
