#!/bin/bash
# [DEPRECATED] 這個 script 原本用 HTCondor 在 batch 節點上依 step=1..5 執行 cmsRun。
# 現在建議改用 CRAB（見 5_sub_crab.sh），此檔保留僅供參考，不再用於大量生產.
...
then
    # MINIAOD
    cd /afs/cern.ch/work/p/pelai/HZa/gridpacks/genfield/massProduce/run3/CMSSW_13_0_13/src/
    cmsenv
    cd HZaTo2l2g_M${mass}/fraction${fraction}
    export X509_USER_PROXY=/tmp/x509up_u175325
